...
impl Writer {
    ...
    fn update_cursor(&self) {
        let cursor_pos = self.row_position * BUFFER_WIDTH + self.column_position;
        ...
        unsafe {
            ...
            cmd_port.write(0x0F);
            data_port.write((cursor_pos & 0xFF) as u8);
            
            // Cursor location high byte
            cmd_port.write(0x0E);
            data_port.write(((cursor_pos >> 8) & 0xFF) as u8);
        }
    }
...
pub fn move_cursor_up() {
    WRITER.lock().move_cursor_up();
}

pub fn move_cursor_down() {
    WRITER.lock().move_cursor_down();
}
